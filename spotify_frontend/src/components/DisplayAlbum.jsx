    ...
    return albumData ? <>
        ...
        <hr />
        {
            songsData.filter((item) => item.album === album.name).map((item, i) => (
                <div onClick={() => playWithId(item._id)} className="grid grid-cols-3 sm:grid-cols-4 gap-2 p-2 items-center text-[#a7a7a7] hover:bg-[#ffffff26] cursor-pointer" key={item._id}>
